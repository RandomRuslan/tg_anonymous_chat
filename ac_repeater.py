    ...
    def _do_match(self, user_id, processed_queue, queues):
        ...
        while user_set_for_choice:
            ...
            if random_user_id != user_id:
                if user_id not in self.manager.users[random_user_id].last_chats and \
                        random_user_id not in self.manager.users[user_id].last_chats:

                    self.manager.pairs[user_id] = random_user_id
                    self.manager.pairs[random_user_id] = user_id

                    self.manager.users[user_id].last_chats.append(random_user_id)
                    self.manager.users[random_user_id].last_chats.append(user_id)
                    if len(self.manager.users[user_id].last_chats) > User.MAX_LAST_CHATS:
                        self.manager.users[user_id].last_chats.pop(0)
                    if len(self.manager.users[random_user_id].last_chats) > User.MAX_LAST_CHATS:
                        self.manager.users[random_user_id].last_chats.pop(0)
...
class Cleaner:
    ...
    def clean_inactive_users(self):
        ...
        for user_id in users:
            ...
            if not self.manager.pairs.get(user_id) and now - user.last_activity_ts > self.ALLOWED_INACTIVITY_PERIOD:
                del self.manager.users[user_id]

                if user_id in self.manager.queue[user.queue_key]:
